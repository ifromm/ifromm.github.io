...
def is_author_selected(entry, names, select_field=''):
    ...
    if "author" in entry:
        ...
        author_names = entry['editor'].split(', ')
...
def get_title_citation_url(scholarID):
    ...
    #    print(u'https://scholar.google.com/%s' % i['href'])
    #print("***")
...
def get_bibtex_from_entry(entry, comma_to_and=False):
    ...
    if comma_to_and:
        if "author" in entry2:
            ...
            authors = entry2['editor'].split(', ')
            entry2['author'] = ' and '.join(authors)
            entry2['author'] = entry2['author'] + " (eds)"
    entry_standard = {}
...
def write_entries_by_year(bib_entries, show_total_citation=False):
    ...
    for e in bib_entries:
        year = e.get('year')
        #print("Year = ",  year, e)
        if year in year_entries_dict:
            year_entries_dict[year].append(e)
        else:
            year_entries_dict[year]=[e]
